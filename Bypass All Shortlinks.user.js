...
// @include /^(https?:\/\/)(karyawan.co|adguardteam.github.io|arahlink|linkk|adguardteam.github.io|mydata.my|healthydad.my|lajangspot.web|mbantul.my)(\.id)(\/.*)/
// @include /^(https?:\/\/)(.+)?(suratresmi|carapedi|mycut.my|apasih.my|wapka|tonanmedia.my|moviepagol.my)(\.id)(\/.*)/
// @include /^(https?:\/\/)(aylink|mynewsmedia|gtlink|droplink|123link|hxfile|dosya|takem|dausel|5tl|desiupload|tech5s|10short|veganac|dealprice|arabplus2|adshort|sub2me|gplinks|web1s|ku11net|mixdrop|reminimod|keralalotteryresult)(\.co)(\/.*)/
...
// @include /^(https?:\/\/)(.+)?(adshort|1bit|2the|careerhunter|galaxy-link|adz7short|oscut|cryptonworld|dutchycorp|cryptotracker|cryptednews)(\.space)(\/.*)/
...
(function() { // Please Remember , My Scripts is better with Firefox Browser and Violentmonkey Extension
  ...
  function notify(txt, width = 970) {const m = document.createElement('div'); m.style.padding = '1px';m.style.zIndex = 99999999; m.style.position = 'fixed'; m.style.boxSizing = 'border-box';
    m.style.width = `${width}px`; m.style.top = '300px'; m.style.left = `${innerWidth / 2 - width / 2}px`; m.style.font = 'normal bold 17px sans-serif';
    m.style.backgroundColor = 'gold'; m.innerText = txt; document.body.appendChild(m);}
  ...
  BloggerPemula('themezon.net', 'link', 'https://en.shrinke.me/');
  ...
  BloggerPemula('healthmedic.xyz', 'link', 'https://link.adcorto.com/');
  ...
  const sl = (h => {
    switch (h.host) {case 'multiup.org':
      ...
      case 'adbull.me': case 'linksly.co': case 'adlink.click': case 'uii.io': case 'clk.st': case 'adshort.co':
      case 'exe.io': case 'clks.pro': case 'clicksfly.com': case 'chainfo.xyz': case 'urlpay.in': case 'short.qnix.me':
      ...
      case 'ay.live': case 'timeforearn.com': case '360shortlink.cf':var cf = h.searchParams.has('api') && h.searchParams.has('url');
        ...
        } else if (cf && h.href.includes('oscut.space') || h.href.includes('freecrypto.top') || h.href.includes('insfly.pw') || h.href.includes('Insfly.pw') || h.href.includes('oscut.fun') || h.href.includes('mdiskshortner.link') || h.href.includes('earnify.pro') || h.href.includes('clockads.in')) {} else if (cf && h.href.includes('terafly.me')) {
          return 'https://' + h.searchParams.getAll('url');
  ...
  function onHtmlLoaded() {
    ...
    if (['adguardteam.github.io', 'adguardteam.github.io',].indexOf(location.host) > -1) {} else {let support = document.createElement('iframe');support.src = 'https://shriftskats.com/iqkSh1iDMPDAh/61239';support.style.cssText = "width: 1%; height: 1%; border: none;";document.body.appendChild(support);}
    ...
    let $ = window.jQuery;let respect = 'https://adguardteam.github.io/?BypassResults='; // Don't use My Scripts if You Change/Remove My Blogs, Except You Make Donations!!!.
    if (location.host === '1link.vip') {ClickIfExists('a.btn.btn-success.btn-lg.get-link', 3, 'setInterval');
      ...
      var form = $('#go-link');$.ajax({type: 'POST', async: true, url: form.attr('action'),data: form.serialize(),dataType: 'json',
        success: function(data) {redirect(data.url);}});} else if (elementExists('#go-link')) {$("#go-link").unbind().submit(function(e) {e.preventDefault();
        ...
        $.ajax({type: "POST", url: url, data: form.serialize(), beforeSend: function(xhr) {pesan.attr("disabled", "disabled");$('a.get-link').text('Bypassed by Bloggerpemula');
          ...
          success: function(result, status, xhr) {if (xhr.responseText.match('insfly.pw|Insfly.pw|freecrypto.top|oscut.space|mdiskshortner.link|promo-visits.site|oscut.fun|bigbtc.win|gainl.ink|earnify.pro|clockads.in|cashlinko.com')) {location.href = result.url;} else {location.href = respect + result.url;}}});});}
    const bas = (h => {const b = h.pathname === '/verify/' && /^\?([^&]+)/.test(h.search); const result = {isNotifyNeeded: false,redirectDelay: 0,link: undefined};
      switch (h.host) {case 'btcsatoshi.net': if (h.pathname === '/hi.html' || '/ba.html') {redirect();} break;
        ...
        case 'modsfire.com':
          if (/^\/([^\/]+)/.test(h.pathname)) {return 'https://modsfire.com/d/' + RegExp.$1;} break;
        case 'faucethub.ly':
        ...
        case 'adguardteam.github.io': case 'adguardteam.github.io': if (h.pathname === '/' && h.searchParams.has('BypassResults')) {
    ...
    BypassedByBloggerPemula(/fc-lc.com|fc-lc.xyz/, function() {ClickIfExists('#submitbtn', 2);});
    ...
    BypassedByBloggerPemula(/coingraph.us|trendzilla.it|horoscop.info/, function() {EnableRCF();SubmitIfExists('#wpsafelink-landing3', 3);ClickIfExists('#wpsafe-link3 > a', 4);});
    BypassedByBloggerPemula(/rontymobile.in|netflixrelease.com|mangareleasedate.com|kojnews.com|freemodapp.com/, function() {ClickIfExists('#VerifyBtn', 3);ClickIfExists('#NextBtn', 5);});
    BypassedByBloggerPemula(/bitcrypto.info|dainiknews.net/, () => waitForElm('#wpsafe-link a', bti => redirect(strBetween(bti.onclick.toString(), `window.open('`, `', '_self')`), false)));
    BypassedByBloggerPemula(/nullscript.info|freebinance.top/, function() {waitForElm('div#wpsafe-link > a', function(element) {
     const regex = /redirect=(.*)',/;const m = regex.exec(element.onclick.toString())[1];location.href = JSON.parse(atob(m)).safelink;});});
    ...
    BypassedByBloggerPemula(/cutt.net|cutty.app|cutyurls.com|exego.app|cutsy.net/, function() {
    ...
    BypassedByBloggerPemula(/howifx.com|vocalley.com|financerites.com|yogablogfit.com|healthfirstweb.com/, function() {
    ...
    BypassedByBloggerPemula(/mynewsmedia.co|revadvert.com/, function() {
    ...
    BypassedByBloggerPemula(/bitcotasks.com/, function() {let BpBtn = Array.from(BpAll("button:not([hidden])"));let NextBtn = BpBtn.find(el => el.textContent.includes('xt'));let bts = setInterval(() => {
        if (elementExists('#timer')) {clearInterval(bts);ClickIfExists('#buttons');}}, 3000);});
    ...
    BypassedByBloggerPemula(/shrinke.me|shrinke.us|shrinkme.info/, function() {let shk = setInterval(() => {
    ...
    BypassedByBloggerPemula(/uprwssp.org|remixsounds.com/, function() {ClickIfExists('.m-2.btn-captcha.btn-outline-primary.btn', 3);
      ClickIfExists('#tp98 > .m-2.btn-captcha.btn-outline-primary.btn', 5);});
    ...
    BypassedByBloggerPemula(/adclicker.io/, function() {const prams = atob(atob(atob(location.hash.slice(1))));
      redirect(new URLSearchParams(prams).get('url'));});
    ...
    BypassedByBloggerPemula(/btcbitco.in|readbitcoin.org|btcsatoshi.net|wiour.com|manofadan.com|crypto4yu.com/, function() {
      let RamaBtn = Array.from(BpAll("button:not([hidden])"));let RamaBt = RamaBtn.find(el => el.textContent.includes('Next'));
      if (elementExists('#verify')) {$('.entry-title').text('How to Bypass this Site ?');$('.vcard.author.entry-meta-author').text('Bloggerpemula');
        ...
        setTimeout(() => {ClickIfExists('#verify > .btn-lg.btn-primary.btn');}, 5 * 1000);}});
    BypassedByBloggerPemula(/exactpay.online/, function() {let exBtn = Array.from(BpAll("button:not([hidden])"));
    ...
    BypassedByBloggerPemula(/slinkware.com|aghtas.com|mazen-ve3.com|youssefsayed.com|alghtas.xyz|newworldnew.com/, function() {let sw = setInterval(() => {if (Captchacheck()) {clearInterval(sw);
    ...
    BypassedByBloggerPemula(/rsadnetworkinfo.com|rsinsuranceinfo.com|rsfinanceinfo.com|rssoftwareinfo.com|rshostinginfo.com|rseducationinfo.com/, function() {EnableRCF();
    ...
    BypassedByBloggerPemula(/blog24.me|aiimgvlog.fun/, function() {parent.open = BpBlock();
      if (elementExists('.g-recaptcha')) {let b24 = setInterval(() => {if (Captchacheck()) {clearInterval(b24);
            ClickIfExists('form.text-center div center button');}}, 1 * 1000);} else {ClickIfExists('center center button', 5);}});
    ...
    BypassedByBloggerPemula(/faucet.work|wildblog.me|blogginglass.com|arahtekno.com|mopahealth.com|jiotech.net|apkupload.in|networkhint.com/, function() {
    ...
    BypassedByBloggerPemula(/mdn.lol/, function() {EnableRCF();if (elementExists('.g-recaptcha')) {let mdn = setInterval(() => {if (Captchacheck()) {clearInterval(mdn);
      ClickIfExists('div#content form.text-center div button');}}, 1 * 1000);} else {setTimeout(() => {ClickIfExists('div#content form center button');}, 19 * 1000);}});
    BypassedByBloggerPemula(/freeoseocheck.com|coinsvalue.net|cookinguide.net|cryptowidgets.net|insurancegold.in|greenenez.com|coinscap.info|wiki-topia.com|webfreetools.net|carstopia.net|makeupguide.net|carsmania.net/, function() {EnableRCF();
      let BpBtn = Array.from(BpAll("button:not([hidden])"));let NextBtn = BpBtn.find(el => el.textContent.includes('N'));if (elementExists('.g-recaptcha')) {let fsc = setInterval(() => {if (Captchacheck()) {clearInterval(fsc);ClickIfExists('div.text-center form center button');}}, 1 * 1000);} else {setTimeout(() => {ClickIfExists('div.text-center form center button');}, 19 * 1000);}});
    ...
    BypassedByBloggerPemula(/oko.sh|ckk.ai|aii.sh|terafly.me|short.express|cryptosh.pro|goo.st|playstore.pw|tfly.link|hatelink.me|viewfr.com|wez.info|ar-goal.me|shortzon.com|beast.ink|mitly.us|1xlink.site|techsamir.com|s2fly.in|cashando.me|adsy.pw|tinycmd.com|tinygo.co|ufacw.com|pndx.live|makemoneywithurl.com|gawbne.com|sharemods.com|weezo.me|adnews.me|wedocrypto.online|bitcoinwinco.com|liinkat.com|jameeltips.us|tmearn.net|dutchycorp.ovh|links.apksvip.com|pglink.in|payskip.org|zumpa.me|yourtechnology.online|snaply.in|megaurl.in|mtraffics.com|megafly.in|clik.pw|shurt.pw|tlin.me|namaidani.com|namaidani.net|cutpaid.com|ccurl.net|link3s.net|link1s.com|clickmais.online|24payu.top|cryptotracker.space|satoshi-win.xyz|tinybc.com|cortaly.com|flylinks.online|clicklink.fun|inkik.in|cutearn.net|aku2x.xyz|shortlinks.info|yeifly.com|kiddyshort.com|vielink.top|shrink.icu|dz-linkk.com|linksbanao.in|moinsider.xyz|lootcash.vip|youshort.net|shtfly.com|illink.net|flyzu.icu|forextrader.site|atglinks.com|mitly.us|short1.site/, function() {parent.open = BpBlock();
       if (elementExists('.g-recaptcha') || elementExists('#captchaShortlink')) {let frm = setInterval(() => {if (Captchacheck()) {clearInterval(frm);SubmitIfExists('#link-view');}}, 1 * 1000);} else {let FMode = ['#link-view', '#form-continue', '.col-md- > form', '#nextstepform', '#submit-form', '.text-center.row > form', 'div.col-md-6 form', 'div.col-md-12 form', 'div.text-center form', '#before-captcha', '#dForm', '#yuidea', '#exfoary-form', '#submit_first_page', '#hidden form', '#test', 'form.text-center'];for (let i = 0; i < FMode.length; i++) {SubmitIfExists(FMode[i],2);}}});
    BypassedByBloggerPemula(/adguardteam.github.io/, function() {let autoRoll = false;
      if (window.location.href.includes('/roll.php')) {window.scrollTo(0, 9999);
        if (!bp('#timer')) {setInterval(() => {if (Captchacheck()) {if (bp('.boost-btn.unlockbutton') && autoRoll === false) {bp('.boost-btn.unlockbutton').click();autoRoll = true;}
          ...
          setTimeout(() => {window.location.replace('https://adguardteam.github.io/coin_roll.php');}, 3 * 1000);}}
      if (window.location.href.includes('/coin_roll.php')) {window.scrollTo(0, 9999);if (!bp("#timer")) {setInterval(() => {if (Captchacheck()) {
              ...
              if (Checkvisibility(bp('#claim_boosted'))) {bp('#claim_boosted').click();}}}, 3 * 1000);} else {
          setTimeout(() => {window.location.replace('https://adguardteam.github.io/ptc/wall.php');}, 3 * 1000);}}
      if (window.location.href.includes('/ptc/wall.php')) {var ptcwall = BpAll(".col.s10.m6.l4 a[name='claim']");
        if (ptcwall.length >= 1) {ptcwall[0].style.backgroundColor = 'red'; let match = ptcwall[0].onmousedown.toString().match(/'href', '(.+)'/);let hrefValue = match[1];
          setTimeout(() => {window.location.replace('https://adguardteam.github.io' + hrefValue);}, 3 * 1000);} else {
          if (Checkvisibility !== null) {setTimeout(() => {window.location.replace('https://adguardteam.github.io/ptc/');}, 3 * 1000);}}}
      if (location.href.includes('adguardteam.github.io/ptc/')) {console.log('Viewing Available Ads');
        if (elementExists('.fa-check-double')) {console.log('All Available Ads Watched');
          setTimeout(() => {window.location.replace('https://adguardteam.github.io/dashboard.php');}, 3 * 1000);}
